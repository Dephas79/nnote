...
get_all(UserID) ->
    ...
    {ok, {_,List_of_matching_objs_keys,_,_}} = riakc_pb_socket:get_index(Pid, <<"nnote">>, {binary_index,Index_name_}, term_to_binary(UserID)),
...
get_records_by_date(UserID, Type, Date) ->
    ...
    List = lists:map(F,List_of_matching_objs_keys),
    List2 = [N||N <- List,N#nnote.type >= Type,qdate:between(FirstDate, N#nnote.date, LastDate)],
    n_common:close_connection(Pid),
    List2.
